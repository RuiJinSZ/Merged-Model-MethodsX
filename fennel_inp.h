...
!
!svn $Id: redoxh_inp.h 810 2016-08-05 01:58:16Z arango $
!================================================== Hernan G. Arango ===
      ...
      DO WHILE (.TRUE.)
        ...
        IF (status.gt.0) THEN
          SELECT CASE (TRIM(KeyWord))
            ...
            CASE ('LDeNSR')
              Npts=load_r(Nval, Rval, Ngrids, LDeNSR)  
            CASE ('LDeRRC')
            ...
            CASE ('SDeNSR')
              Npts=load_r(Nval, Rval, Ngrids, SDeNSR)  
            CASE ('SDeRRC')
            ...
            CASE ('ElDON')
              Npts=load_r(Nval, Rval, Ngrids, ElDON)
            CASE ('EsDON')
              Npts=load_r(Nval, Rval, Ngrids, EsDON)
            CASE ('deltN')
              Npts=load_r(Nval, Rval, Ngrids, deltN)
            CASE ('a0N')
              Npts=load_r(Nval, Rval, Ngrids, a0N)  
            CASE ('O2palFrac')
              Npts=load_r(Nval, Rval, Ngrids, O2palFrac)
            CASE('rkd1')
              Npts=load_r(Nval, Rval, Ngrids, rkd1)
            CASE('rkdChl1')
              Npts=load_r(Nval, Rval, Ngrids, rkdChl1)
            CASE('rkdTSS1')
              Npts=load_r(Nval, Rval, Ngrids, rkdTSS1)
            CASE('rkdS1')
              Npts=load_r(Nval, Rval, Ngrids, rkdS1)          
            CASE ('TNU2')
...
# ifdef USECOS_BURIAL
            CASE ('Dout(iNbur)')
              IF (iDbio2(iNbur).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iNbur)'
                  exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
            CASE ('Dout(iNbot)')
              IF (iDbio2(iNbot).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iNbot)'
                  exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO  
            CASE ('Dout(iCbot)')
              IF (iDbio2(iCbot).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iCbot)'
                  exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
            CASE ('Dout(iCbur)')
              IF (iDbio2(iCbur).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iCbur)'
                  exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
              CASE ('Dout(iPbot)')
              IF (iDbio2(iPbot).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iPbot)'
                  exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
              CASE ('Dout(iPbur)')
              IF (iDbio2(iPbur).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iPbur)'
                  exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
...
# ifdef H_SULF
            CASE ('Dout(iSRR_)') 
              IF (iDbio2(iSRR_).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iSRR_)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
            CASE ('Dout(iBFlx)')
              IF (iDbio2(iBFlx).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iBFlx)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
            CASE ('Dout(iSRRb)') 
              IF (iDbio2(iSRRb).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iSRRb)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
...
# endif
            ...
            CASE ('Dout(iNO3u)')
              ...
              END DO
	    CASE ('Dout(iNFix)')
              IF (iDbio3(iNFix).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iNFix)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
      CASE ('Dout(iPO4u)')
              IF (iDbio3(iPO4u).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iPO4u)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
...
# ifdef H_SULF
            CASE ('Dout(iSOx_)')
              IF (iDbio3(iSOx_).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iSOx_)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iOxic)') 
              IF (iDbio3(iOxic).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iOxic)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iSRRa)')
              IF (iDbio3(iSRRa).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iSRRa)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iDno2)')
              IF (iDbio3(iDno2).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iDno2)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
            CASE ('Dout(iAnx_)')
              IF (iDbio3(iAnx_).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iAnx_)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iNit1)')
              IF (iDbio3(iNit1).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iNit1)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iNit2)')
              IF (iDbio3(iNit2).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iNit2)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iDno3)')
              IF (iDbio3(iDno3).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iDno3)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iSNx_)') 
              IF (iDbio3(iSNx_).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iSNx_)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
	    CASE ('Dout(iSNy_)')
              IF (iDbio3(iSNy_).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iSNy_)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
...
!
     ...
     &            'Large detritus N re-mineralization rate (day-1).'
            WRITE (out,80) LDeNSR(ng), 'LDeNSR',                        &
     &            'Solubilization rate for large detritus N at 0C (day-1).'
     ...
     &            'Remineralization rate for small detritus N (day-1).'
            WRITE (out,80) SDeNSR(ng), 'SDeNSR',                        &
     &            'Solubilization rate for small detritus N at 0C (day-1).'
     ...
     &            'CO2 partial pressure in air (ppm by volume).'
            WRITE (out,80) ElDON(ng), 'ElDON',                          &
     &            'Phytoplankton exudation rate of labile DON (day-1).'
            WRITE (out,80) EsDON(ng), 'EsDON',                          &
     &            'Phytoplankton exudation rate semilabile DON (day-1)'
            WRITE (out,90) deltN(ng), 'deltN',                          &
     &            'Fraction of semilabile DON to total DON in the ',    &
     &            'phyto cell (nondimensional).'
            WRITE (out,80) a0N(ng), 'a0N',                              &
     &            'Remineralization rate of DON at 0 deg.C (day-1)'
            WRITE (out,80) O2palFrac(ng), 'O2palFrac',                  &
     &            'Fraction of O2 PAL (nondimensional).'
            WRITE (out,80) rkd1(ng), 'rkd1',                            &
     &        'light attenuation by low salinity seawater in Chespeake Bay (meter-1).' 
            WRITE (out,80) rkdChl1(ng), 'rkdChl1',                      &
     &        'light attenuation by chlorophyll at low salinity seawater in Chespeake Bay (meter^2 mg_Chl-1).'
            WRITE (out,80) rkdTSS1(ng), 'rkdTSS1',                      &
     &        'light attenuation by TSS at low salinity seawater in Chespeake Bay (meter^2 g-1).'
            WRITE (out,80) rkdS1(ng), 'rkdS1',                          &
     &        'light attenuation by CDOM (use salinity instead) at low salinity seawater in Chespeake Bay (meter-1 PSU-1).'
#ifdef TS_DIF2
            DO itrc=1,NBT
              i=idbio(itrc)
...
!
  ...
  60  FORMAT (/,/,' Redoxh Model Parameters, Grid: ',i2.2,              &
     &        /,  ' =================================',/)
